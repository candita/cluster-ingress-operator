...
func Test_Reconcile(t *testing.T) {
	...
	gw := func(name string, listeners ...gatewayapi.Listener) *gatewayapi.Gateway {
	...
	l := func(name, hostname string, port int) gatewayapi.Listener {
		h := gatewayapi.Hostname(hostname)
		return gatewayapi.Listener{
			Name:     gatewayapi.SectionName(name),
			Hostname: &h,
			Port:     gatewayapi.PortNumber(port),
		}
	}
...
func Test_gatewayListenersHostnamesChanged(t *testing.T) {
	l := func(name, hostname string) gatewayapi.Listener {
		h := gatewayapi.Hostname(hostname)
		return gatewayapi.Listener{
			Name:     gatewayapi.SectionName(name),
			Hostname: &h,
	...
	tests := []struct {
		name     string
		old, new []gatewayapi.Listener
		expect   bool
	}{
		{
			name:   "no listeners",
			old:    []gatewayapi.Listener{},
			new:    []gatewayapi.Listener{},
			expect: false,
		},
		{
		...
		},
		{
			name:   "add a listener",
			old:    []gatewayapi.Listener{},
			new:    []gatewayapi.Listener{l("http", "xyz.xyz")},
			expect: true,
		},
		{
			name:   "remove a listener",
			old:    []gatewayapi.Listener{l("http", "xyz.xyz")},
			new:    []gatewayapi.Listener{},
			expect: true,
		},
		{
			...
			old:    []gatewayapi.Listener{l("http", "xyz.xyz")},
			new:    []gatewayapi.Listener{l("https", "xyz.xyz")},
			expect: true,
		},
		{
			name:   "change a listener's hostname",
			old:    []gatewayapi.Listener{l("https", "xyz.xyz")},
			new:    []gatewayapi.Listener{l("https", "abc.xyz")},
			expect: true,
		},
		{
