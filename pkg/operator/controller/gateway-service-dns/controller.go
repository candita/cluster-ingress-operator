...
func NewUnmanaged(mgr manager.Manager, config Config) (controller.Controller, error) {
	...
	gatewayListenersChanged := predicate.Funcs{
		UpdateFunc: func(e event.UpdateEvent) bool {
			old := e.ObjectOld.(*gatewayapi.Gateway).Spec.Listeners
			new := e.ObjectNew.(*gatewayapi.Gateway).Spec.Listeners
	...
	if err := c.Watch(source.Kind[client.Object](operatorCache, &gatewayapi.Gateway{}, handler.EnqueueRequestsFromMapFunc(gatewayToService), isInOperandNamespace, gatewayListenersChanged)); err != nil {
		return nil, err
	}
...
func gatewayListenersHostnamesChanged(xs, ys []gatewayapi.Listener) bool {
	x := map[string]string{}
	y := map[string]string{}
	for i := range xs {
...
func getGatewayHostnames(gateway *gatewayapi.Gateway) sets.String {
...
func (r *reconciler) ensureDNSRecordsForGateway(ctx context.Context, gateway *gatewayapi.Gateway, service *corev1.Service, domains []string, infraConfig *configv1.Infrastructure, dnsConfig *configv1.DNS) []error {
...
func (r *reconciler) deleteStaleDNSRecordsForGateway(ctx context.Context, gateway *gatewayapi.Gateway, service *corev1.Service, domains sets.String) []error {
	listOpts := []client.ListOption{
